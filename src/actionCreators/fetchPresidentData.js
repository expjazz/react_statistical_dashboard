...
export const fetchPresidentData = () => async (dispatch, getState) => {
  ...
  try {
    let image = null;
    const [rawText, rawImage] = await Promise.all([fetch(`https://en.wikipedia.org/w/api.php?format=json&action=query&prop=extracts&exintro&explaintext&redirects=1&titles=${president}&origin=*`), fetch(`https://en.wikipedia.org/w/api.php?action=query&format=json&formatversion=2&prop=pageimages|pageterms&piprop=original&titles=${president}&origin=*`)]);
